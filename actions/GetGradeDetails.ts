...
function calculateStdDeviation(grades: number[], avg: number): number {
	if (grades.length === 0) return 0;
	const variance =
		grades.reduce((acc, grade) => acc + Math.pow(grade - avg, 2), 0) /
		grades.length;
	return Math.sqrt(variance);
}

function calculateDistribution(grades: number[]): {
	labels: string[];
	counts: number[];
} {
	const labels = [
...
async function GetAndCalculateGradeDetails(
	...
	try {
		...
		logger.info("Failed to fetch grade statistics", {
			code: grade.code,
