...
export default function LootCase({
	grade,
	onComplete,
	onTick,
	onReveal,
	...
	return (
		<div className="flex flex-col items-center gap-4">
			<div
				ref={containerRef}
				className={`relative w-[90vw] md:w-[600px] h-[120px] md:h[160px] overflow-hidden border-4 border-backgroundTertiary dark:border-backgroundTertiary rounded-xl bg-backgroundSecondary
					...
					{items.map((item, idx) => (
						<div
							...
							className={`flex items-center justify-center text-white flex-shrink-0 relative ${
								showWinnerGlow && idx === WINNING_INDEX ? "winner-glow" : ""
							}`}
							style={{
								width: ITEM_WIDTH,
								height: "100%",
								...
								boxShadow:
									idx === WINNING_INDEX && showWinnerGlow
										? `0 0 18px 6px ${item.color}, inset 0 0 10px 4px white`
			...
			<style jsx>{`
				...
				.winner-glow {
					box-shadow: 0 0 15px 5px ${items[WINNING_INDEX]?.color || "yellow"},
