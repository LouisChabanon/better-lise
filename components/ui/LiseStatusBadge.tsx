...
export default function LiseStatusBadge({
	discrete = false,
}: {
	discrete?: boolean;
}) {
	...
	if (isLowData) {
		statusConfig = {
			...
			description:
				"Pas assez de connexions dans les deux derniéres heures pour évaluer.",
	...
	return (
		...
		>
			{/* BADGE */}
			<div
				className={`inline-flex items-center gap-2.5 px-3 py-1.5 rounded-full border cursor-help transition-colors duration-300 ${statusConfig.bgColor} ${statusConfig.borderColor}`}
			>
				...
				<div
					className={`text-xs font-medium ${statusConfig.textColor} flex leading-none gap-0.5`}
				>
					<span className="opacity-90 hidden sm:inline">Lise :</span>
				...
				{isHovered && (
					...
					>
						...
						<div className="flex items-start gap-3 mb-3">
							<div
								className={`p-2 rounded-lg ${statusConfig.bgColor} ${statusConfig.textColor}`}
							>
								{statusConfig.icon}
							</div>
							<div>
								...
								<p className="text-xs text-textTertiary leading-tight mt-1">
						...
						{!isLowData && (
							<div className="space-y-2 pt-3 border-t border-border/50">
								<div className="flex justify-between items-center text-xs">
									...
									<span
										className={`font-mono font-semibold ${statusConfig.textColor}`}
									>
