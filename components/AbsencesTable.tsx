...
export function AbsencesTable({ session }: { session: any }) {
	...
	useEffect(() => {
		...
		const filtered = localAbsences.filter(
			(a) =>
						...
						<div className="overflow-y-auto p-4 space-y-3">
							{stats.length === 0 ? (
								...
								stats.map((stat, idx) => (
									<div
										key={idx}
										className="flex flex-col bg-backgroundSecondary/50 rounded-xl p-3 border border-primary"
									>
										<div className="flex justify-between items-start mb-2">
											...
											<div
												className={`px-2 py-1 rounded-lg text-xs font-bold ${
													(stat?.percentage || 0) >= 20
														? "bg-red-100 text-red-700"
														: (stat?.percentage || 0) >= 10
										...
										<div className="flex justify-between text-[10px] text-textTertiary">
											...
											<span>Total Module: {stat?.totalUE}h</span>
										</div>
									</div>
								))
							)}
										...
										<tbody className="text-textSecondary divide-y divide-calendarGridBorder">
											...
											{desktopAbsences.map((a, index) => (
												...
												>
													...
													<td
														className="px-4 py-4 align-top truncate text-textTertiary"
														title={a.intervenants}
													>
														{a.intervenants}
													</td>
													<td
														className="px-4 py-4 align-top text-textTertiary truncate"
														title={a.motif}
													>
														{a.motif || "Aucun"}
													</td>
