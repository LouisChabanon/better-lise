...
export function GradeTable({ session, gambling }: GradeTableProps) {
  ...
  const isFilterActive = selectedSemester !== "all" || selectedAssignment !== "all" || selectedTeacher !== "all";
  ...
  const processedGrades = useMemo(() => {
    ...
    if (selectedSemester !== "all") {
      result = result.filter(g => getSemesterFromCode(g.code) === selectedSemester);
    }

    // 3. Assignment Filter
    if (selectedAssignment !== "all") {
      result = result.filter(g => getAssignmentFromCode(g.code) === selectedAssignment);
    }
    ...
    return result;
  }, [localGrades, searchTerm, selectedSemester, selectedAssignment, selectedTeacher, sortConfig]);
  ...
  const currentGrades = processedGrades.slice(startIndex, startIndex + pageSize);
  ...
  const SortIcon = ({ columnKey }: { columnKey: keyof GradeType }) => {
      if (sortConfig.key !== columnKey) return <div className="opacity-50"><FilterOutlined /></div>;
      return sortConfig.direction === 'asc' ? <CaretUpFilled /> : <CaretDownFilled />;
  };
      ...
      <div className="flex flex-col gap-3 mb-4 shrink-0 relative z-20">
          ...
          <div className="flex flex-row gap-2 items-center">
            ...
            <Button 
                ...
                className={`shrink-0 !px-3 ${isFilterActive && !isFiltersOpen ? "ring-2 ring-primary/30" : ""}`}
            >
            ...
            <Button status="primary" onClick={() => {
                refetch()
                if (posthog.has_opted_in_capturing()) posthog.capture("grades_refresh");
            }} disabled={isFetching} className="shrink-0 !px-3">
             ...
             <div className="bg-backgroundPrimary border border-buttonSecondaryBorder rounded-xl p-4 shadow-lg animate-in slide-in-from-top-2 fade-in duration-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] uppercase font-bold text-textTertiary">Semestre</label>
                        <select 
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] uppercase font-bold text-textTertiary">Matière</label>
                        <select 
                            className="px-3 py-2 border border-buttonSecondaryBorder bg-backgroundSecondary rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
                    ...
                    {/* Teacher Select */}
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] uppercase font-bold text-textTertiary">Enseignant</label>
                        <select 
                            className="px-3 py-2 border border-buttonSecondaryBorder bg-backgroundSecondary rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
                    ...
                    <div className="flex flex-col gap-1 sm:hidden">
                        <label className="text-[10px] uppercase font-bold text-textTertiary">Trier par</label>
                        <select 
                            className="px-3 py-2 border border-buttonSecondaryBorder bg-backgroundSecondary rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
                            value={`${sortConfig.key}-${sortConfig.direction}`}
                            onChange={(e) => {
                                const [key, dir] = e.target.value.split('-');
                                setSortConfig({ key: key as keyof GradeType, direction: dir as 'asc' | 'desc' });
                            }}
                        >
                ...
                {hasNewGrades && (
                    <div className="mt-4 pt-3 border-t border-border/50 flex justify-end">
                        <Button status="secondary" onClick={handleMarkAllRead} className="w-full sm:w-auto">
                    ...
                    >
                      ...
                      <div className="flex-1 pr-4 min-w-0">
                        ...
                        <div className="flex flex-col gap-1 text-xs text-textTertiary">
                          ...
                          {g.teachers && <span className="flex items-center gap-1 truncate text-textQuaternary"><UserOutlined /> {g.teachers}</span>}
                        </div>
                      </div>
                ...
                <div className="hidden md:block flex-1 overflow-auto rounded-t-lg bg-backgroundPrimary border border-backgroundSecondary">
                  <table className="table-fixed min-w-full text-sm divide-y divide-gray-200">
                    <thead className="bg-backgroundTertiary uppercase text-xs font-semibold z-10 sticky top-0">
                      <tr>
                        <th className="px-4 py-3 text-left w-1/3 cursor-pointer hover:bg-backgroundSecondary/80 group select-none" onClick={() => handleSort('libelle')}>
                            <div className="flex items-center gap-2">
                                Libellé <SortIcon columnKey="libelle" />
                            </div>
                        </th>
                        <th className="px-4 py-3 text-left w-24 cursor-pointer hover:bg-backgroundSecondary/80 group select-none" onClick={() => handleSort('note')}>
                             <div className="flex items-center gap-2">
                                Note <SortIcon columnKey="note" />
                            </div>
                        </th>
                        <th className="px-4 py-3 text-left w-32 cursor-pointer hover:bg-backgroundSecondary/80 group select-none" onClick={() => handleSort('date')}>
                            <div className="flex items-center gap-2">
                                Date <SortIcon columnKey="date" />
                            </div>
                        </th>
                        <th className="px-4 py-3 text-left w-32 cursor-pointer hover:bg-backgroundSecondary/80 group select-none" onClick={() => handleSort('code')}>
                             <div className="flex items-center gap-2">
                                Code <SortIcon columnKey="code" />
                            </div>
                        </th>
                         <th className="px-4 py-3 text-left w-40 cursor-pointer hover:bg-backgroundSecondary/80 group select-none" onClick={() => handleSort('teachers')}>
                             <div className="flex items-center gap-2">
                      ...
                      {currentGrades.map((g) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-textQuaternary text-xs truncate" title={g.teachers}>{g.teachers}</td>
                          </tr>
                        );
                      })}
                    </tbody>
