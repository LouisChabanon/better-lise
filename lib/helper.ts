...
function CalculateEatingTime(events: CalendarEventProps[]) {
	...
	const byDay: Record<
		string,
		{ indices: number[]; events: CalendarEventProps[] }
	> = {};
	...
	for (const dayKey of Object.keys(byDay)) {
		...
		if (nonRu.length === 0) {
			...
			const duration =
				(mealWindowEnd.getTime() - mealWindowStart.getTime()) / (1000 * 60);
			if (duration >= MIN_MEAL_DURATION_MINUTES)
				gaps.push({ start: mealWindowStart, end: mealWindowEnd });
		} else {
			// before first
			const first = nonRu[0];
			...
			for (let i = 0; i < nonRu.length - 1; i++) {
				...
				if (
					(e2.getTime() - s2.getTime()) / (1000 * 60) >=
					MIN_MEAL_DURATION_MINUTES
				)
					gaps.push({ start: s2, end: e2 });
			}

			// after last
			const last = nonRu[nonRu.length - 1];
			const s3 =
				last.endDate > mealWindowStart ? last.endDate : mealWindowStart;
			const e3 = new Date(year, month, day, 23, 59, 59);
			const e3clamped = e3 < mealWindowEnd ? e3 : mealWindowEnd;
			if (
				(e3clamped.getTime() - s3.getTime()) / (1000 * 60) >=
				MIN_MEAL_DURATION_MINUTES
			)
				gaps.push({ start: s3, end: e3clamped });
		}
		...
		for (let k = 0; k < ruLocalIndices.length; k++) {
			...
			const newStart =
				gap.start > mealWindowStart ? gap.start : mealWindowStart;
...
export function CalculateOverlaps(events: CalendarEventProps[]) {
	...
	const sorted = [...adjustedEvents].sort(
